GrassmannOptim <- function (objfun, W, sim_anneal = FALSE, temp_init = 20, cooling_rate = 2, 
          max_iter_sa = 100, eps_conv = 1e-05, max_iter = 100, eps_grad = 1e-05, 
          eps_f = .Machine$double.eps, verbose = FALSE) 
{
  call <- match.call()
  if ((is.null(W$Qt)) & (is.null(W$dim))) 
  ...
  orthonorm <- function(u) {
    ...
    if (p > 1) {
      for (i in 2:p) {
        coef.proj <- c(crossprod(u[, i], v[, 1:(i - 
                                                  1)]))/diag(crossprod(v[, 1:(i - 1)]))
  ...
  max_objfun <- function(All_Qt, W) {
    ...
    L[abs(L) == Inf] = NA
    if (sum(is.na(L)) == nlength) 
      return(list(status = "allNA"))
    index <- min(which(L == max(L, na.rm = TRUE)))
    return(list(Qt = All_Qt[index][[1]], L = round(L[index], 
                                                   digits = 5), index = index, status = "OK"))
  }
  if (verbose) 
    cat("Initialization...", "\n")
  if (sim_anneal) {
    seq_delta <- runif(1) * exp(seq(-10, 0, by = 2)) %x% 
      c(-1, 1)
    ...
    while (temperature > 0.1) {
      for (i in 1:max_iter_sa) {
        ...
        ws <- matrix(rnorm(d * (p - d)), nrow = d, ncol = (p - 
                                                             d))
        temp_alpha <- getGradient(objfun, W, fvalue, 
                                  eps_grad) + sqrt(temperature) * ws
        alpha[1:d, (d + 1):p] <- temp_alpha
        matA <- GetA(alpha, p, d)
        candidates_Qt <- vector("list")
        Expms <- lapply(seq_delta, function(delta) matrix(attributes(Matrix::expm(Matrix::Matrix(-delta * 
                                                                                   matA)))$x, nrow = nrow(matA), ncol = ncol(matA)))
        ...
        if (gridmax$status != "allNA") {
          ...
          selector <- as.numeric(runif(1) < min(diff_ratio, 
                                                1))
          newQt <- gridmax$Qt
          Qt <- selector * newQt + (1 - selector) * 
            Qt
        }
      }
  ...
  if ((norm_grad <= eps_conv)) {
    converged = TRUE
    return(invisible(list(Qt = round(Qt, digits = 4), d = d, 
                          norm_grads = norm_grads, fvalues = fvalues, converged = converged, 
                          call = call)))
  }
  repeat {
    iter = iter + 1
    ...
    temp_alpha <- getGradient(objfun, candidate_W, candidate_fvalue, 
                              eps_grad)
    alpha[1:d, (d + 1):p] <- temp_alpha
    norm_grad <- sum(diag(t(alpha) %*% alpha))
    if ((norm_grad <= eps_conv)) {
      if (verbose) {
        cat(sprintf("%4.0i\t%1.4e\t%1.4e\t", iter, candidate_fvalue, 
                    norm_grad), "\n")
      }
      converged = TRUE
      break
    ...
    if (iter >= max_iter) {
      if (verbose) 
        message("Convergence may not have been reached.\nMaximum iterations is reached")
      converged = FALSE
      break
    }
  }
